...
router.post('/dmv/list', middleware.LoggedInMember, middleware.ProvideCommunity, middleware.ProvideCivilianAuth, async (req, res) => {
    const data = await CAD.query(`SELECT * FROM vehicles WHERE civilian_id = ? AND community_id = ?`, [req.civilian, req.community])
    res.status(200).json(data[0])
})

router.post('/dmv/add', middleware.LoggedInMember, middleware.ProvideCommunity, middleware.ProvideCivilianAuth, async (req, res) => {
...
router.post('/dmv/edit', middleware.LoggedInMember, middleware.ProvideCommunity, middleware.ProvideCivilianAuth, async (req, res) => {
...
router.post('/dmv/delete', middleware.LoggedInMember, middleware.ProvideCommunity, middleware.ProvideCivilianAuth, async (req, res) => {
    const vehicleID = req.body.vehicle_id
    if (!vehicleID) {
        res.status(400).send('You must provide a vehicle ID.')
        return
